...
<div class="content_luar">
  <div class="content_dalam">
    ...
    <form name="frealisasi" id="frealisasi" action="<?= site_url()."/tools/revisidokumenpabean" ?>" method="post" autocomplete="off">
	  <input type="hidden" name="TIPE" id="TIPE" value="GATE-OUT" />
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td>Lokasi Penimbunan Barang</td>
          <td>
            <?php
			/*$func = get_instance();
			$func->load->model("main","main", true);
			$KODE_LOKASI = $func->main->get_combobox("SELECT KODE_LOKASI, NAMA_LOKASI FROM M_TRADER_LOKASI_BRG 
			WHERE KODE_TRADER='".$this->newsession->userdata('KODE_TRADER')."' ORDER BY NAMA_LOKASI ","KODE_LOKASI", "NAMA_LOKASI", FALSE);
           $KODELOKASI =  $KODE_LOKASI?array_merge(array('PUSAT'=>'PUSAT'),$KODE_LOKASI):array('PUSAT'=>'PUSAT');
           echo form_dropdown('KODE_LOKASI', $KODELOKASI, $sess['KODE_LOKASI'], 'id="KODE_LOKASI" class="text"  wajib="yes" '.$disabled); */
           ?></td>
        </tr>-->
      <tr>
        <td colspan="4">&nbsp;</td>
