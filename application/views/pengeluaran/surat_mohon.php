...
?>
<a href="javascript:void(0)" onclick="window.location.href='<?= site_url()."/pengeluaran/pemusnahan/surat/".$tujuan?>'" style="float:right;margin:5px 0px 0px 0px" class="btn btn-success btn-sm prev" id="ok_">
 	...
 	<td colspan="2">
       ...
       <?php if($act=="save" || !$DETILBARANG){ ?>
        <tr id="tr_barang">
            ...
            <combo><?= form_dropdown('DTLSURAT[KODE_GUDANG][]', array_merge(array(""=>"- Pilih -"),$GUDANG), '', 'id="KODE_GUDANG1" wajib="yes" onChange="getRak(1)" class="sstext" '.$disabled);?></combo>
            </td>
            <td id="tdRak1" class="alt2 bright">
            <combo><?= form_dropdown('DTLSURAT[KODE_RAK][]', array_merge(array(""=>"- Pilih -"),$RAK), '', 'id="KODE_RAK1" onChange="getSubRak(1)" class="sstext" '.$disabled);?></combo>
            </td>
...
<tr>
	<td colspan="2">
    <a href="javascript:void(0);" class="btn btn-success btn-l save" id="ok_" onclick="save_post('#fsurat_');"><span><i class="fa fa-save"></i>&nbsp;<?= ucwords($act); ?>&nbsp;</span></a>&nbsp;<a href="javascript:;" class="btn btn-warning btn-l cancel" id="cancel_" onclick="cancel('fsurat_');"><span><i class="icon-undo"></i>&nbsp;Reset&nbsp;</span></a><span class="msg_" style="margin-left:20px">&nbsp;</span>
	</td>
</tr>
</table>
</form>
...
function getRak(id){
	...
	var kdGudang = $("#tdGudang"+id+" #KODE_GUDANG"+id).val();
	$.ajax({
		type: 'post',
		url: site_url + '/pengeluaran/getRak',
		data: {kode_barang:kdBrg, jns_barang: jnsBrg, kode_gudang: kdGudang, tdId: id},
		success: function(data) {
			$("table#tableBarang td#tdRak"+id).html(data);  
		}
	});
	
}

function getSubrak(id){
	var kdBrg = $("#tdBarang"+id+" #KODE_BARANGSRT"+id).val();
    var jnsBrg = $("#tdJenisBarang"+id+" #JNS_BARANG"+id).val();
	var kdGudang = $("#tdGudang"+id+" #KODE_GUDANG"+id).val();
	var kdRak = $("#tdRak"+id+" #KODE_RAK"+id).val();
	$.ajax({
		type: 'post',
		url: site_url + '/pengeluaran/getSubrak',
		data: {kode_barang:kdBrg, jns_barang: jnsBrg, kode_gudang:kdGudang, kode_rak:kdRak, tdId:id},
		success: function(data) {
			$("table#tableBarang td#tdSubRak"+id).html(data);  
		}
	});
}
FormReady();
