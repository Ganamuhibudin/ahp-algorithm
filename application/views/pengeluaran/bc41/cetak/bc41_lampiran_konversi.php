<?php
function headerLampKonv($NOMOR_AJU,$KODE_KPBC,$URAIAN_KPBC,$URJENIS_TPB,$URTUJUAN_KIRIM,$NOMOR_PENDAFTARAN,$TANGGAL_PENDAFTARAN)
{
	...
	$content .='<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="92%" align="center" class="border-b" height="33pt"><strong>LEMBAR LAMPIRAN<br>KONVERSI PEMAKAIAN BAHAN</strong></td>
				</tr>
				<tr>
	...
	$content .="<tr>";
	$content .="<td>&nbsp; A.</td>";
	$content .="<td width=\"15%\">KANTOR PABEAN </td>";
	$content .="<td>&nbsp;</td>";
	$content .="<td>&nbsp;</td>";
	$content .="<td>&nbsp;</td>";
	$content .="<td>&nbsp;</td>";
	...
	$content .='<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td colspan="10"><strong>NOMOR PENGAJUAN</strong> :'.FormatAju($NOMOR_AJU).'</td>
				</tr>   
				<tr> 
					<td width="55%" valign="top"> 
						...
						<tr>
							...
							<td width="69%" valign="top">'.$KODE_KPBC.' '.ucwords(strtolower($URAIAN_KPBC)).'</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>:</td>
							<td>'. ucwords(strtolower($URTUJUAN_KIRIM)).'</td>
						</tr>
						</table>
					</td>    
	...
	$content .= "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\">";
	$content .= "<tr>";
	$content .= "<td align=\"center\" colspan='4' class=\"border-brt\" width=\"50%\">Barang Jadi</td>";
	$content .= "<td align=\"center\" colspan='3' class=\"border-b border-t\" width=\"50%\">Bahan Baku yang digunakan</td>";
	$content .= "</tr>";
	$content .= "<tr>";
	$content .=  "<td width=\"2%\" valign=\"top\" class=\"border-br\">No</td>";
	$content .= "<td width=\"30%\" valign=\"top\" class=\"border-br\">Pos Tarif /HS uraian jumlah dan jenis barang secara <br>lengkap, kode barang,merek, tpe, ukuran dan spesifikasi<br> lain</td>";
...
function footerLampKonv($KOTA_TTD,$TANGGAL_TTD,$NAMA_TTD){
	...
	$footer .="<tr>";
	$footer .="	<td class=\"border-b\" width=\"50%\">&nbsp;&nbsp;</td>";
	$footer .="	<td class=\"border-b\" width=\"50%\"><strong>E TANDA TANGAN PENGUSAHA TPB</strong></td>";
	$footer .="</tr>";
	$footer .="<tr>";
	...
	$footer .="</tr>";
	$footer .="<tr>";
	$footer .="	<td align=\"center\" >".$KOTA_TTD.", tgl ".$TANGGAL_TTD."</td>";
	$footer .="</tr>";
	$footer .="<tr>";
	$footer .="	<td align=\"center\" >&nbsp;</td>";
	$footer .="</tr>";
	...
	$this->db->from('T_BC41_BB');
	$jumTotBB= $this->db->count_all_results();
	//echo 'Total Bahan Baku :'.$jumTotBB;
	echo headerLampKonv($DATA['NOMOR_AJU'],$DATA['KODE_KPBC'],$DATA['URAIAN_KPBC'],$DATA['URJENIS_TPB'],$DATA['URTUJUAN_KIRIM'],$DATA['NOMOR_PENDAFTARAN'],$DATA['TANGGAL_PENDAFTARAN']);
	if($jumTotBB>1){
	  $no=1; $nm=1;
      foreach($BARANG as $barJd):	 	  
		if($no%11==0){
			echo "<pagebreak>";
			echo headerLampKonv($DATA['NOMOR_AJU'],$DATA['KODE_KPBC'],$DATA['URAIAN_KPBC'],$DATA['URJENIS_TPB'],$DATA['URTUJUAN_KIRIM'],$DATA['NOMOR_PENDAFTARAN'],$DATA['TANGGAL_PENDAFTARAN']);
		}
	  
		$query = $this->db->get_where('T_BC41_BB', array('NOMOR_AJU' => $barJd['NOMOR_AJU'],'SERI' => $barJd['SERI']), '', '');
		
		$this->db->where(array('NOMOR_AJU' => $barJd['NOMOR_AJU'],'SERI' => $barJd['SERI']));
		$this->db->from('T_BC41_BB');
	  ...
	  <?
		if($jumBB > 0){
            ...
            <? } ?>            
				...
				<?= //11+($nexjumBB?$nexjumBB:0).'<-'.$x.'<>'.$nexjumBB.'<='.
					$row->KODE_HS_BB.'<br>'.$row->URAIAN_BARANG_BB.'<br>'.$row->KODE_BARANG_BB.',
					'.$row->MERK_BB.','.$row->TIPE_BB.','.$row->UKURAN_BB.',<br>'.$row->SPF_BB;
				?>
				</td>
				<td class="border-br"  valign="top"><?=$row->JUMLAH_SATUAN_BB;?></td>
				<td class="border-b"  valign="top"><?=$row->KODE_SATUAN_BB;?></td>
			</tr>
		...
		?>
				...
				<td class="border-br" valign="top">&nbsp;</td>
				<td class="border-br" valign="top">&nbsp;</td>
				<td class="border-b" valign="top">&nbsp;</td>
			</tr>
