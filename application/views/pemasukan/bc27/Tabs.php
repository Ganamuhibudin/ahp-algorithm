...
<?php if($aju){ ?>
<a href="javascript:void(0)" onclick="Cekstatus('<?=site_url()."/pemasukan/cekstatus/".$aju."/bc27"?>');" style="float:right;margin:4px 4px 0px 0px" class="btn btn-sm btn-warning"><i class="icon-info-sign"></i> Cek Status</a>    
...
<?php } ?>
<div class="header">
	<h3 class="green"><i class="fa fa-file-text-o"></i>&nbsp;<strong><?=$judul?></strong></h3>
</div>
<div class="content">
  <div class="tabbable">
    ...
    <div class="tab-content">
      <div id="tabHeader" class="tab-pane active">
        <?php 
			$func->load->model("bc27/header_act");
			$data = $func->header_act->get_header($aju);
			$this->load->view("pemasukan/bc27/Header",$data);	
		?>
      </div>
      <div id="tabBarang" class="tab-pane">
        <?php
			if ($aju) {
				$func->load->model("bc27/detil_act");
				$arrdata = $func->detil_act->detil('barang', $dokumen, $aju, 'edit');
				$list = $this->load->view('view', $arrdata, true);
			}
			$func->load->model("bc27/barang_act");
			$data = $func->barang_act->get_barang($aju);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc27/Barang",$data);
		?>
      </div>
      <div id="tabBarangJadi" class="tab-pane">
        <?php 		
			if($aju){			
				$func->load->model("bc27/detil_act");	
				$arrdata = $func->detil_act->detil('barang_jadi',$dokumen,$aju,'edit');
	 			$list = $this->load->view('view', $arrdata, true); 
			}
			$func->load->model("bc27/barang_jadi_act");
			$data = $func->barang_jadi_act->get_barang_jadi($aju);	
			if($aju)$data = array_merge($data,array('list' => $list)); 		
			$this->load->view("pemasukan/bc27/Barang_jadi", $data);	
		?>
      </div>
      <div id="tabKemasan" class="tab-pane">
        <?php
			...
			$func->load->model("bc27/kemasan_act");
			$data = $func->kemasan_act->get_kemasan($aju, $seri);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc27/Kemasan",$data);
		?>
      </div>
      <div id="tabKontainer" class="tab-pane">
        <?php
			...
			$func->load->model("bc27/kontainer_act");
			$data = $func->kontainer_act->get_kontainer($aju, $seri);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc27/Kontainer",$data);
		?>
      </div>
