...
<?php if(!$list){?>
    <form id="fkemasan_" action="<?= site_url()."/pemasukan/kemasan/bc27"; ?>" method="post" autocomplete="off" list="<?= site_url()."/pemasukan/detil/kemasan/bc27" ?>">
        ...
        <table>
            ...
            <tr>
                <td>Jumlah </td>
                <td><input type="text" name="JUM" id="JUM" class="text" maxlength="18" wajib="yes" onkeyup="this.value = ThausandSeperator('JUMLAH',this.value,4);" value="<?= $this->fungsi->FormatRupiah($sess['JUMLAH'],4); ?>"/><input type="hidden" name="KEMASAN[JUMLAH]" id="JUMLAH" value="<?= $sess['JUMLAH']?>" /></td>
            </tr>
            <tr>
                <td>Jenis Kemasan </td>
                    <td><div class="input-group" style="float:left; width:3em; margin-bottom:0px">
                            ...
                            <span class="input-group-btn">
                                <a href="javascript:void(0)" name="cari" id="cari" class="btn btn-primary btn-xs" style="vertical-align:top;height:27px " onclick="tb_search('kemasan','KODE_KEMASAN;urjenis_kemasan','Kode Kemasan','fkemasan_',650,400)"><i class="fa fa-ellipsis-h"></i></a>
                            </span>
                        </div>&nbsp;<span id="urjenis_kemasan" class="uraian"><?= $sess['URAIAN_KEMASAN']; ?></span></td>
            </tr> 
            <tr>
                <td colspan="2">&nbsp;</td>
            </tr>
