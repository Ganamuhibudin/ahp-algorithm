...
<?php if($aju){ ?>
<a href="javascript:void(0)" onclick="Cekstatus('<?=site_url()."/pemasukan/cekstatus/".$aju."/bc33"?>');" style="float:right;margin:4px 4px 0px 0px" class="btn btn-sm btn-warning"><i class="icon-info-sign"></i> Cek Status</a>    
    ...
    <div class="tab-content">
      ...
      <div id="tabKemasan" class="tab-pane">
        <?php
			...
			$func->load->model("bc33/kemasan_act");
			$data = $func->kemasan_act->get_kemasan($aju, $seri);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc33/Kemasan",$data);
		?>
      </div>
      ...
      <div id="tabDokumen" class="tab-pane">
         <?php
			...
			$func->load->model("bc33/dokumen_act");
			$data = $func->dokumen_act->get_dokumen($aju, $seri);
			if ($aju)
            $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc33/Dokumen",$data);
		?>
      </div>
    </div>
  </div>
...
<?php } ?>
function change_merk(){
	var merk = $('#fbc33_ #MERK_KEMASAN').val();
	
	$('#fkemasan_ #MERK_KEMASAN').val(merk);
}
</script>