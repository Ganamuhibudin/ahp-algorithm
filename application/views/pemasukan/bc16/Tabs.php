...
<?php if($aju){ ?>
<a href="javascript:void(0)" onclick="Cekstatus('<?=site_url()."/pemasukan/cekstatus/".$aju."/bc16"?>');" style="float:right;margin:4px 4px 0px 0px" class="btn btn-sm btn-warning"><i class="icon-info-sign"></i> Cek Status</a>    
    ...
    <div class="tab-content">
      ...
      <div id="tabKemasan" class="tab-pane">
        <?php
			...
			$func->load->model("bc16/kemasan_act");
			$data = $func->kemasan_act->get_kemasan($aju, $seri);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc16/Kemasan",$data);
		?>
      </div>
      <div id="tabKontainer" class="tab-pane">
        <?php
			...
			$func->load->model("bc16/kontainer_act");
			$data = $func->kontainer_act->get_kontainer($aju, $seri);
			if ($aju) $data = array_merge($data, array('list' => $list));
        	$this->load->view("pemasukan/bc16/Kontainer",$data);
		?>
      </div>
