...
<div id="div1">
  <table class="tableA">
    <tr>
      <th colspan="2" align="center">Upload Data Sub Rak<span style="float:right;"><b>PLBInventory</b>&nbsp;</span></th>
    </tr>
  </table>
  ...
  <div style="padding:0 10px 0 10px;">
	<?php 	
		if($proses){
			echo "<br>";
			if($proses=="error"){
				echo '<input type="button" class="button" value="Upload Ulang" style="height:32px;" onclick="location.href=\''.site_url().'/uploadnew/subrak\'">&nbsp;<input type="button" class="button" value="  Batal  " onClick="self.close();" style="height:32px;">';
			}else{
				echo '<input type="button" class="button" value="Upload Lagi" style="height:32px;" onclick="location.href=\''.site_url().'/uploadnew/subrak\'">&nbsp;<input type="button" class="button" value="  Tutup  " onClick="self.close();" style="height:32px;">';	
			}
			echo $msg;
		}else{
    ?>
   <?php echo $this->session->flashdata('txtuploadexcel'); ?>
    <form method="post" name="frmUpload" enctype="multipart/form-data" id="frmUpload" action="<?php echo site_url('uploadnew/proses_subrak');  ?>" onSubmit="return cekFileKirim(this.name); return false;">
      <br />
      <a href="<?php echo base_url().'assets/file/UploadDokSubRak.xls';?>" style="text-decoration:none;"><img src='<?php echo base_url(); ?>img/tbl_xls.png' align="absmiddle">&nbsp;Contoh Format Data</a>
      <br />
      <br />
